...
const WED = {
  ...
  expenses: [
    { id:1, category:'venue',       label:'Venue deposit',        amount:50000, paid:true  },
    { id:2, category:'catering',    label:'Catering package',     amount:80000, paid:true  },
    { id:3, category:'florals',     label:'Floral arrangements',  amount:25000, paid:false },
    { id:4, category:'photography', label:'Photo & video package',amount:45000, paid:true  },
    { id:5, category:'attire',      label:'Bridal gown',          amount:30000, paid:false },
    { id:6, category:'music',       label:'Live band',            amount:20000, paid:false },
    { id:7, category:'cake',        label:'Wedding cake',         amount:8000,  paid:false },
    { id:8, category:'invites',     label:'Invitations printing', amount:5000,  paid:true  },
  ],

  checklist: [
    { phase:'12 Months Out', items:[
      { id:'c1',  text:'Set wedding date',                done:true  },
      { id:'c2',  text:'Book the venue',                  done:true  },
      { id:'c3',  text:'Set overall budget',              done:true  },
      { id:'c4',  text:'Create initial guest list',       done:true  },
      { id:'c5',  text:'Hire wedding coordinator',        done:false },
    ]},
    { phase:'6 Months Out', items:[
      { id:'c6',  text:'Book photographer & videographer',done:true  },
      { id:'c7',  text:'Book catering',                   done:true  },
  ...
  schedule: [
    { time:'6:00 AM',  event:'Hair & Makeup — Bridal party',       assignee:'Glam Team',        done:false, color:'pink' },
    { time:'9:00 AM',  event:'Groom & groomsmen prep',             assignee:'Carlo (best man)',  done:false, color:'sand' },
    { time:'11:00 AM', event:'Venue setup & florals',              assignee:'Coordinator',       done:false, color:'green'},
    { time:'12:00 PM', event:'Photographer & videographer arrive', assignee:'Studio XYZ',        done:false, color:'sand' },
    { time:'2:00 PM',  event:'Guest arrival & cocktail hour',      assignee:'Front-of-house',    done:false, color:'cream'},
    { time:'3:00 PM',  event:'Ceremony',                           assignee:'Officiant',         done:false, color:'pink' },
    { time:'4:00 PM',  event:'Photo session — couple',             assignee:'Studio XYZ',        done:false, color:'sand' },
    { time:'5:00 PM',  event:'Reception & dinner',                 assignee:'Catering team',     done:false, color:'green'},
    { time:'7:00 PM',  event:'Program — speeches & dances',        assignee:'Emcee',             done:false, color:'cream'},
    { time:'9:00 PM',  event:'Cake cutting & dessert',             assignee:'Catering team',     done:false, color:'pink' },
    { time:'10:00 PM', event:'Send-off & confetti exit',           assignee:'All guests',        done:false, color:'sand' },
  ],

  // Canvas furniture items
  furniture: [
    { id:'f1', type:'stage',    x:160, y:20,  w:140, h:60,  label:'Stage',          emoji:'🎭' },
    { id:'f2', type:'round',    x:40,  y:120, w:70,  h:70,  label:'Table 1 (8)',     emoji:'⭕' },
    { id:'f3', type:'round',    x:140, y:120, w:70,  h:70,  label:'Table 2 (8)',     emoji:'⭕' },
    { id:'f4', type:'round',    x:240, y:120, w:70,  h:70,  label:'Table 3 (8)',     emoji:'⭕' },
    { id:'f5', type:'round',    x:40,  y:220, w:70,  h:70,  label:'Table 4 (8)',     emoji:'⭕' },
    { id:'f6', type:'long',     x:160, y:230, w:120, h:50,  label:'Head Table',      emoji:'▬' },
    { id:'f7', type:'entrance', x:170, y:320, w:80,  h:36,  label:'Entrance',        emoji:'🚪' },
  ],
...
function openChairGuestPicker(chairId) {
  ...
  overlay.style.cssText = [
    'position:fixed;inset:0;z-index:900;background:rgba(44,31,14,0.28);',
    'display:flex;align-items:flex-end;justify-content:center;'
  ].join('');

  const rows = WED.guests.map(g => {
    const isSeated = seated.has(g.id);
    const isCurrent = currentGuest && g.id === currentGuest.id;
    return `<div onclick="assignChairGuest('${chairId}',${g.id})" style="
  ...
  overlay.innerHTML = `
    <div style="
      ...
      background:var(--cream);border-radius:var(--r-xl) var(--r-xl) 0 0;
      padding:20px 16px 32px;max-height:70vh;overflow-y:auto;
    ...
    ">
      ...
      ${currentGuest?`<button onclick="assignChairGuest('${chairId}',null)" style="width:100%;padding:9px;border-radius:var(--r-md);background:rgba(224,120,152,0.12);border:1px solid rgba(224,120,152,0.25);font-size:12.5px;font-weight:700;color:var(--pink-deep);cursor:pointer;margin-bottom:10px">🗑 Remove ${currentGuest.name.split(' ')[0]}</button>`:''}
