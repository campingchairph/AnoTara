...
function renderOverview() {
  ...
  // Toggle action button: + if not set up, ✏️ if set up
  const actionBtn = document.getElementById('wed-overview-action-btn');
  const isSetUp = WED.couple.p1 && WED.couple.p2;
  if (actionBtn) {
    actionBtn.textContent = isSetUp ? '✏️' : '➕';
    actionBtn.title = isSetUp ? 'Edit Details' : 'Set Up Wedding';
  }
