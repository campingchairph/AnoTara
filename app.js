...
function getPIN()  { return localStorage.getItem('at_pin') || ''; }
...
function showPinLock(mode) {
  ...
  el.classList.add('active');
  updatePinDots();
  const lbl   = document.getElementById('pin-lock-label');
  const hint  = document.getElementById('pin-setup-hint');
  const errEl = document.getElementById('pin-error');
  if (errEl) errEl.textContent = '';
  if (_pinMode === 'setup') {
    if (lbl)  lbl.textContent  = 'Choose a 4-digit PIN';
    if (hint) hint.style.display = 'block';
  } else if (_pinMode === 'confirm') {
    if (lbl)  lbl.textContent  = 'Confirm your PIN';
    if (hint) hint.style.display = 'none';
  } else {
    const u = AUTH?.user?.name?.split(' ')[0] || 'Jhoan';
    if (lbl)  lbl.textContent  = 'Welcome back, '+u+'!';
    if (hint) hint.style.display = 'none';
  }
}
  ...
  if (_pinMode === 'setup') {
    ...
    if (_pinBuffer === getPIN()) {
      ...
      if (errEl) {
        ...
        screen.style.animation = 'shake 0.3s';
