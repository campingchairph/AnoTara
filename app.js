...
/* ── NAVIGATION ──────────────────────────────── */
const ALL_SCREENS = ['home','groups','group-detail','couple','receipt','settings','wedding','daily-tasks','mood-board','suppliers','errand-board'];
const NAV_IDS     = ['home','groups','couple','settings'];
const FAB_HIDDEN  = ['group-detail','receipt','settings','wedding','daily-tasks','mood-board','suppliers','errand-board'];
...
function switchTab(name) {
  ['tasks','expenses','members','settle','map','saan-tayo'].forEach(t => {
    const panel = document.getElementById('panel-'+t);
    const tab   = document.getElementById('tab-'+t === 'tab-saan-tayo' ? 'tab-saan' : 'tab-'+t);
    if (panel) panel.style.display = t === name ? 'block' : 'none';
  });
  // handle tab active states separately (saan-tayo tab id is tab-saan)
  ['tasks','expenses','members','settle','map'].forEach(t => {
    const tab = document.getElementById('tab-'+t);
    if (tab) tab.classList.toggle('active', t === name);
  });
  const saanTab = document.getElementById('tab-saan');
  if (saanTab) saanTab.classList.toggle('active', name === 'saan-tayo');
  if (name === 'map') initMap();
  if (name === 'saan-tayo') {
    const group = getActiveGroup();
    renderGroupSuggestions(group);
...
function renderGroupsList() {
  ...
  // Full groups list
  const list = document.getElementById('groups-list-el');
  if (list) {
...
function submitErrandItem() {
  ...
  list.items.push({ id:newId, name, qty, price, aisle, note, photo:'', done:false, skipped:false });
  saveErrands();
